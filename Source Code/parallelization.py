    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            if line[0] != '1' and line[0] != '2':  # isolate the name of the satellite
                sat = list(CelestrakAPI.load_gp_from_celestrak(name=line))[0]
                tle = CelestrakAPI.extract_tle(sat)
...
'''
    header = ['Satellite Name', 'Time', 'Position']
    with open('parallelData.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        for i in range(len((tle_array))):
            writer.writerow(name_array[i])
            writer.writerow(tle_array[i])
        writer.writerow([])
    ...
    for i in range(len((tle_array))):
        ...
        with open('parallelData.csv', 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([name_array[i][0], datetime.datetime.now(), updated_position[i]])
